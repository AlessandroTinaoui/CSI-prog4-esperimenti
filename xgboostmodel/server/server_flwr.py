...
def main():
    ...
    # 1) carica ensemble globale salvato dalla strategy
    with open("global_ensemble.pkl", "rb") as f:
    ...
    if HOLDOUT_CID <= 8:
        ...
        # predizione con ensemble (media pesata)
        dmat = xgb.DMatrix(X_holdout)
        preds = []
        weights = []
        for item in ensemble_items:
            booster = xgb.Booster()
            booster.load_model(item["raw"])
